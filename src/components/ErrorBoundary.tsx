...
const ErrorFallback = ({ error }: FallbackProps) => {
  ...
  const rawError = error ? (
    typeof error === 'object' ? 
      `Raw error object: ${Object.getOwnPropertyNames(error).map(prop => `${prop}: ${String((error as any)[prop])}`).join(', ')}` : 
...
export function ErrorBoundary() {
  ...
  try {
    ...
    console.warn('ErrorBoundary: Not in a router context, using fallback error handler');
  }
  
  // If not in a router context, use the error from props
  if (!isInRouterContext) {
    return <ErrorFallback error={new Error('Component error occurred outside router context')} />;
  }
  ...
  const rawError = routeError ? (
    typeof routeError === 'object' ? 
      `Raw error object: ${Object.getOwnPropertyNames(routeError).map(prop => `${prop}: ${String((routeError as any)[prop])}`).join(', ')}` : 
