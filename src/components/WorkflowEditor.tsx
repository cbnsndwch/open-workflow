...
const WorkflowEditor: React.FC<WorkflowEditorProps> = ({
  ...
  const handleWorkflowChange = (updatedWorkflow: WorkflowGraph) => {
    try {
      ...
      if (!validation.valid) {
        toast.warning(`Workflow has ${validation.errors.length} validation issues`);
      }
  ...
  return (
    <div className={`workflow-editor h-full ${fullscreen ? 'fixed inset-0 z-50 bg-background' : ''} ${className || ''}`}>
      {activeWorkflow && (
        <WorkflowVisualizer 
