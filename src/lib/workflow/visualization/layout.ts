...
export function calculateLayout(
    ...
    for (const level of levels.sort((a, b) => a - b)) {
        ...
        const levelWidth =
            nodesInLevel.length * (nodeWidth + nodePadding) - nodePadding;
    ...
    const nodesPerLevelCount = Object.values(nodesPerLevel).map(
        nodes => nodes.length
    );
    const maxNodesPerLevel = Math.max(...nodesPerLevelCount, 1);
    const calculatedWidth =
        maxNodesPerLevel * (nodeWidth + nodePadding) - nodePadding + 80;
